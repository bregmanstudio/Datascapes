...
function setup() {
  ...
  delay = new p5.Delay();
  reverb.process(osc,4);
  delay.process(osc,(random(8)+1)*.125, random(.8)+.2, random(-3000,3000)+2300)  ;
  frameRate(16); // tempo
...
function draw() {
  ...
  synthesize();
  if(random(64)<1){
    delay.delayTime((random(8)+1)*.125);
    delay.feedback(random(.95));
    delay.filter(random(-3000,3000)+4000,random(10)+.05);
    reverb.set(random(10),random(2));
    strokeWeight(random(40));
    fill(random(255),255,255);
    rect(x,0,random(50)+5,2*height);
    noStroke();
  }
}
